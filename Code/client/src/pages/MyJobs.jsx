...
const MyJobs = () => {
    ...
    const fetchJobs = async () => {
        try {
            setIsLoading(true);
            const response = await fetch(`http://localhost:4000/api/myjobs?userId=${encodeURIComponent(userId)}`, {
                method: 'GET',
                headers: {
    ...
    return (
        <div className="max-w-6xl mx-auto p-6">
            <h2 className="text-3xl font-semibold mb-6 text-center text-gray-800">My Jobs</h2>
                    ...
                    <div key={job._id} className="bg-white rounded-lg overflow-hidden shadow-lg transition-transform transform hover:scale-105 hover:shadow-xl">
                        <div className="p-6">
                            ...
                            <div className="mb-4">
                                ...
                                <span className={`inline-block px-3 py-1 text-sm font-medium rounded-full ${getStatusStyle(job.status)}`}>
                                    {job.status || 'Pending'}
                                </span>
                            </div>
